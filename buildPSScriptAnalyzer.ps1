...
function ConvertResxStringsToCsharp {
    ...
    if ($PSCmdlet.ShouldProcess($Destination, 'Create File')) {
        ...
        &{
            ...
            "        private static Lazy<string[]>[] _preferredCultureData;"
            "        private static Dictionary<CultureInfo, Lazy<string[]>> _localizedResources;"
            ""
            "        private static string GetString(int stringIndex)"
            "        {"
            "            for (int i = 0; i < _preferredCultureData.Length; i++)"
            "            {"
            "                var stringData = _preferredCultureData[i].Value[stringIndex];"
            "                if (stringData != null)"
            "                {"
            "                    return stringData;"
            "                }"
            ...
            "                _preferredCultureData = culturePreferredData.ToArray();"
            "            }"
            "        }"
            ""
